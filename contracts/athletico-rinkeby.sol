/**
* Rinkeby version
* Start ICO is date of deployment contract
...
/**
 * @title Contract ATTest token
 * @dev ERC20 compatible token contract
 */
contract ATTestToken is ERC20Pausable {
    string public constant name = "ATTest TOKEN";
...
contract Crowdsale is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for ATTestToken;
    ...
    */
    ...
    // !!!     нижние функции - это для тестовой сети, чтобы получить верифицированный код контракта токена   !!!
    // !!!  в MainNet контракт 

    function setNewTokenContract(address _newContract) public onlyOwner {
        token = ATTestToken (_newContract);
